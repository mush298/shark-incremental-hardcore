const SHARK = {
    ...
    bonuses: {
        fish: [()=>player.shark_level.gte(1),l=>expPow(Decimal.pow(tmp.shark_base,l).mul(l),getCRBoost(5)).root(2).pow(l.gte(11) ? 1.5 : 1).pow(l.gte(15) ? 1.25 : 1).mul(l.gte(19) ? 10 : 1),E(0)],
        prestige: [()=>player.shark_level.gte(20),l=>Decimal.add(1.25,simpleResearchEffect('p1',0)).pow(l.sub(19)).mul(player.shark_level.gte(75) ? 1e6 : 1),E(1)],
...
const SHARK_UPGRADES = {
    s1: {
        ...
        effect: l=> {
            let root = hasResearch('idk') ? 1 : 1.5
       

            return l.add(1).pow(sharkUpgEffect('s3')).root(root)
        },
    ...
    s2: {
        ...
        cost: l => Decimal.pow(10,l.pow(1.25)).mul(1000),
        bulk: x => x.div(1000).log(10).root(1.25).floor().add(1),
    ...
    s3: {
        ...
        cost: l => {
            let x = Decimal.pow(1e3,l.scaleAll("su_s3").pow(1.25)).mul(1e6)
            if (hasResearch('c3')) x = x.root(coreReactorEffect(3))
            return x
        },
        bulk: x => {
            if (hasResearch('c3')) x = x.pow(coreReactorEffect(3))
            return x.div(1e6).log(1e3).root(1.25).scaleAll("su_s3",true).floor().add(1)
        },
    ...
    p1: {
        ...
        cost: l => Decimal.pow(2,l.scaleAll("su_p1").pow(hasResearch('idk2') ? 1.5 : 2)).ceil(),
        bulk: x => x.log(2).root(hasResearch('idk2') ? 1.5 : 2).scaleAll("su_p1", true).floor().add(1),

        curr: "prestige",

        effect: l=>Decimal.pow(10,l.mul(simpleResearchEffect('p5'))).min('eee7'),
        effDesc: x=>formatMult(x,0),
    },
    p2: {
        req: ()=>player.prestige.times>0,

        cost: l => Decimal.pow(hasResearch('idk2') ? 100 : 10,l.pow(Decimal.sub(1.25,researchEffect('p9',0)))).mul(1e3),
        bulk: x => x.div(1e3).log(hasResearch('idk2') ? 100 : 10).root(Decimal.sub(1.25,researchEffect('p9',0))).floor().add(1),
    ...
    p3: {
        ...
        cost: l => {
            let x = Decimal.pow(1e6,l.pow(2)).mul(1e29)
            if (hasResearch('c3')) x = x.root(coreReactorEffect(3))
            return x
        },
        bulk: x => {
            if (hasResearch('c3')) x = x.pow(coreReactorEffect(3))
            return x.div(1e29).log(1e6).root(2).floor().add(1)
        },
