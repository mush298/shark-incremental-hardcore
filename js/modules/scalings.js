const SCALINGS = {
    ...
    su_p1: {
        get amount() { return player.shark_upg.p1 },
...
function getScalingStarts(id) {
    ...
    switch (id) {
        case "shark_level": {
            b[0] = Decimal.add(b[0],sharkUpgEffect('p3',0)).add(player.shark_level.gte(145) ? 3 : 0).add(player.shark_level.gte(283) ? 9 : 0).add(hasDepthMilestone(2,2)?player.explore.depth[2].div(500).overflow(1e6,0.5).floor():0)
            b[1] = Decimal.add(b[1],getCRBoost(3,0)).add(simpleResearchEffect('c10',0)).add(player.shark_level.gte(121) ? 50 : 0)
